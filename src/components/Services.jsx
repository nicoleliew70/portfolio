...
const ServicesSection = () => {
    ...
    useEffect(() => {
        // Fetch the JSON file managed by Decap CMS
        // import.meta.env.BASE_URL resolves to '/portfolio/' in production
                        ...
                        <div key={index} className="bg-white rounded-3xl p-8 shadow-sm hover:shadow-xl transition-all border-2 border-transparent hover:border-sky-200 group">
                            <div className="mb-4">
                                <span className={`px-4 py-1 rounded-full text-xs font-bold uppercase tracking-wider ${service.open ? 'bg-green-100 text-green-600' : 'bg-red-100 text-red-600'}`}>
                                    {service.open ? 'Slots Available' : 'Currently Full'}
                                </span>
                            </div>
                            ...
                            <button className="w-full flex items-center justify-center gap-2 bg-yellow-400 hover:bg-yellow-500 text-gray-900 font-bold py-3 rounded-2xl transition-colors">
