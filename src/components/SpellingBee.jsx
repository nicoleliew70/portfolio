...
import spellingBeeData from '../data/spellingBeeData.json';
import { trackGameStart, trackGameComplete } from '../utils/analytics';

const ROUNDS = 5;

const SpellingBee = ({ currentText, whatsAppLink }) => {
    ...
    return (
      <div className="animate-fadeIn w-full max-w-lg">
        ...
        <h3 className="text-3xl font-bold mb-2">
          {score >= 4 ? t.spellingStar : score >= 3 ? t.greatJob : t.keepPracticing}
        </h3>
    ...
    <div className="w-full max-w-lg animate-fadeIn">
      ...
      <div className="bg-amber-50 rounded-2xl p-6 mb-6 text-center">
        ...
        <p className="text-sm text-amber-700 font-medium mb-2">
          {speechSupported ? t.tapToHear : t.audioNotSupported}
        </p>
      ...
      <form onSubmit={handleSubmit}>
        ...
        />
        {feedback === 'correct' && (
          <p className="text-green-600 text-sm mt-2 font-medium text-center">{t.correct}</p>
        )}
        {feedback === 'wrong' && (
          <p className="text-red-500 text-sm mt-2 font-medium text-center">{t.correctSpelling} <strong>{words[round].word}</strong></p>
        )}
        {!feedback && (
