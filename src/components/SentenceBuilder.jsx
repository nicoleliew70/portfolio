...
import sentenceBuilderData from '../data/sentenceBuilderData.json';
...
const SentenceBuilder = ({ currentText, whatsAppLink }) => {
    ...
    return (
      <div className="animate-fadeIn w-full max-w-lg">
        ...
        <h3 className="text-3xl font-bold mb-2">
          {score >= 4 ? t.brilliant : score >= 3 ? t.greatJob : t.keepPracticing}
        </h3>
  ...
  return (
    <div className="w-full max-w-lg animate-fadeIn">
      ...
      <p className="text-sm text-gray-500 mb-4 text-center">{t.tapWords}</p>
      ...
      <div className={`min-h-[60px] border-2 border-dashed rounded-xl p-3 mb-6 flex flex-wrap gap-2 transition ${
        ...
        {selected.length === 0 && <span className="text-gray-400 text-sm italic">{t.tapWordsBelow}</span>}
        {selected.map((item) => (
          <button
            key={item.id}
      ...
      {feedback === 'wrong' && (
        <p className="text-red-500 text-sm mb-4 font-medium text-center">{t.correct} <strong>{sentences[round].correct}</strong></p>
      )}
